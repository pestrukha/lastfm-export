...
def create_tracks_dataframe(rows):
    # Создаем DataFrame без указания dtype
    df = pandas.DataFrame(rows, columns=["Artist", "Album", "Track", "Plays"])

    # Преобразуем типы вручную
    df["Artist"] = df["Artist"].astype(str)
    df["Album"] = df["Album"].astype(str)
    df["Track"] = df["Track"].astype(str)
    df["Plays"] = pandas.to_numeric(df["Plays"], errors='coerce').fillna(
        0).astype(int)

    # Создаем комбинированные столбцы
    df["ArtistAlbum"] = df["Artist"] + " - " + df["Album"]
